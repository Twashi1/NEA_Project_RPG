...
Input::State Input::GetKeyState(int key)
{
	...
	Log(std::string("Couldn't find key ") + std::to_string(key), Utilities::ERROR::WARNING);
...
Input::State Input::GetMouseState(int button)
{
	...
	Log(std::string("Couldn't find button ") + std::to_string(button), Utilities::ERROR::WARNING);
