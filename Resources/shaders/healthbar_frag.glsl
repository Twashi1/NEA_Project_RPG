...
uniform float u_WaveAmplitude = 0.1f;
uniform float u_WobbleAmplitude = 0.05f;
uniform float u_WobbleSpeed = 0.8f;
uniform float u_WaveOffset = 1.0f - 0.1f - 0.05f;
uniform float u_Wavelength = 5.0f;
uniform float u_Wavespeed = 0.3f;
uniform float u_PI = 3.14159265358f;

uniform sampler2D u_HealthbarTexture;
...
vec4 get_fluid_texture() {
    ...
    if (u_VerticalWave) {
        ...
        float noise_real_pos = (v_texCoord.y + u_Time * u_Wavespeed) * u_Wavelength;
    ...
    vec2 scaled_coords = v_texCoord * u_CellularDensity;
...
void main() {
    ...
    vec4 healthbar_color = texture2D(u_HealthbarTexture, v_texCoord);
    vec4 mask_color = texture2D(u_Mask, v_texCoord);
    vec4 fluid_color = get_fluid_texture();

    color = fluid_color;
    color.a *= (1.0f - mask_color.a);
    color += healthbar_color * (1.0f - alpha);
}